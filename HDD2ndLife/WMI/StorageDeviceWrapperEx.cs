...
public class StorageDeviceWrapperEx : DeviceIoWrapperBase
{
    ...
    {
        var nameBytes = br;
        for (var i = index; i < nameBytes.Length; i++)
        {
            if (nameBytes[i] == 0) // \0
            {
    ...
    public STORAGE_DEVICE_DESCRIPTOR_PARSED_EX StorageGetDeviceProperty()
    {
        ...
        var res = DeviceIoControlHelper.InvokeIoControlUnknownSize(Handle, IOControlCode.StorageQueryProperty, query);
        var descriptor = /*Utils.*/ByteArrayToStruct<STORAGE_DEVICE_DESCRIPTOR>(res, 0);

        var returnValue = new STORAGE_DEVICE_DESCRIPTOR_PARSED_EX
        {
