...
public class DiskSectors : Control  // Try and use the "Lowest" fastest UI access
{
    ...
    public DiskSectors()
    {
        BLOCK_SIZE = 6 * DeviceDpi / 96; // 1 line | 4 spaces for colour  -> etc.
    ...
    // TODO: Make this a jagged array for performance
    ...
    public ulong ScaledClusterCount
    {
        set
        {
            ...
            {
                foreach (BitVector64.Section section in sections)
                {
    ...
    {
        var scale = Math.Max(1, clusterStatus.LongLength / (columnCount * rowCount));
        ...
        for (var row = 0; row < rowCount; row++)
        {
            ...
            for (var column = 0; column < columnCount; column++)
            {
                ...
                for (var count = 0;
                     ...
                     statusOffset++, count++)
                {
                    for (var index = 0; index < CLUSTERS_PER_VECTOR; index++)
                    {
    ...
    public void DrawNodeStatus(Graphics g, BlockStatus status, int x, int y, int nodeSize)
    {
        if (status == BlockStatus.Unused)
        {
            // Just save time drawing transparent !
            return;
        }

        (Pen outer, Brush inner) = status switch
        {
        ...
        var xLoc = x * nodeSize + 1;
        var yLoc = y * nodeSize + 1;
        g.DrawRectangle(outer, xLoc, yLoc, nodeSize - 2, nodeSize - 2);
        g.FillRectangle(inner, xLoc + 1, yLoc + 1, nodeSize - 3, nodeSize - 3);
    }
    ...
    private void DiskSectors_Paint(object sender, PaintEventArgs e)
    {
        ...
        for (var x = 0; x < columnCount; x++)
        {
            ...
            {
                DrawNodeStatus(g, Blocks[x, y], x, y, BLOCK_SIZE);
            }
        }
